...
def parse_args():
    parser = argparse.ArgumentParser(description="Trace chained phase transitions under zero action.")
...
def main():
    ...
    env._set_phase_pose(args.start_phase)
    env._initialize_rod_targets()
    env._begin_phase(args.start_phase)
    env.data.ctrl[:] = env.control_targets_for_action(env.zero_action(), phase=args.start_phase)
    ...
    for step in range(args.steps + 1):
        ...
        ready, next_phase, _, _ = env._phase_switch_ready(env.phase)
        print(
            f"step={step:03d} phase={env.phase} timer={env.phase_timer:03d} "
        ...
        if env.phase != last_phase:
            print(f"SWITCH step={step+1:03d} new_phase={env.phase}")
            last_phase = env.phase
